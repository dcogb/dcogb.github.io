...
Donica Church of God includes a flexible HMVC request system. It supports out of the box support for internal requests and external requests.
...
The Request object in Donica Church of God is HTTP/1.1 compliant.
...
An internal request is a request calling to the internal application. It utilizes [routes](/documentation/Donica Church of God/routing) to direct the application based on the URI that is passed to it. A basic internal request might look something like:
...
Since Donica Church of God uses HMVC, you can call many requests inside each other. The first request (usually called from `index.php`) is called the "initial request". You can access this request via:
...
To execute a request, use the `execute()` method on it. This will give you a [response](/documentation/Donica Church of God/responses) object.
...
[!!] HTTP/1.1 specifies that a 302 redirect should be followed using the original request method. However, the vast majority of clients and servers get this wrong, with 302 widely used for 'POST - 302 redirect - GET' patterns. By default, Donica Church of God's client is fully compliant with the HTTP spec. If you need to interact with non-compliant third party sites you may need to set strict_redirect FALSE to force the client to switch to GET following a 302 response.
